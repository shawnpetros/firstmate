#!/usr/bin/env bash
# Cursor IDE transport for the persistent-cd PreToolUse seatbelt.
...
set +e
printf '%s' "$payload" | "$SCRIPT_DIR/fm-cd-pretool-check.sh" >"$tmp" 2>"$err"
rc=$?
set -e
if [ "$rc" -eq 2 ]; then
  reason=$(tr -d '\r' <"$err" | tr '\n' ' ')
  reason=${reason:-[persistent-cd] blocked}
